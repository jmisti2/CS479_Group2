...
void draw() {
  ...
  if(introScreen){
    ...
    fill(255);
    rect(width/2-90, height/2-50,160,80,10);
    textSize(50);
    fill(0);
    strokeWeight(5);
    ...
    text("Start", width/2-77, height/2+5);
  ...
  if(myPort.available() > 0){
      ...
      if(val != null){
        ...
        if(val.equals("7")) {//left arrow
          ...
          println("sensor pressed : (left arrow)");
          ...
          println("sensor pressed : (down arrow)");

        } else if(val.equals("9")) {//right arrow
          if(pause ==false){
        ...
        else if(val.equals("1")) {//pause button
          if(!paused) {
            paused = true;
          } else {
            paused = false;
